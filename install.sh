...
adminuser=$(mysql -N -e "use CyberPanel; select adminuser from setting where id='1';")
adminpass=$(mysql -N -e "use CyberPanel; select adminpassword from setting where id='1';")
clear
if [ "$adminuser" != "" ]; then
...
sed -i 's@#Banner none@Banner /var/www/html/cpanel/banner.txt@' /etc/ssh/sshd_config
